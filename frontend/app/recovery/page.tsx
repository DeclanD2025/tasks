...
import { getMetric, METRICS, READINESS } from "@/lib/data";
    ...
    <Page title="Recovery" eyebrow="Understand · how ready you are today">
      ...
      <Card className="p-5">
        <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center" style={domainStyle("recovery")}>
          <Ring value={READINESS.latest} domain="recovery" size={132} label="/ 100" />
          <div className="min-w-0 flex-1 text-center sm:text-left">
            <div className="text-[11px] font-semibold uppercase tracking-wide domain-text">Readiness · moderate</div>
            <p className="mt-1 max-w-md text-[15px] leading-relaxed text-text">{READINESS.interpretation}</p>
            <div className="mt-3 flex flex-wrap justify-center gap-2 sm:justify-start">
              <Button href="/insights/metric/readiness" variant="ghost" size="sm">See how this is calculated</Button>
              <Meta className="self-center">{READINESS.freshness}</Meta>
            </div>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-4 sm:p-5">
        ...
        <ul className="divide-y divide-border">
          {READINESS.facts.map((f) => {
            const drag = f.detail?.includes("drag") || f.detail?.includes("down");
            return (
              <li key={f.label} className="flex items-center justify-between gap-4 py-2.5">
                ...
                <span className={`tnum shrink-0 text-[15px] font-semibold ${drag ? "text-warn" : "text-good"}`}>{f.value}</span>
              </li>
            );
          })}
        </ul>
      ...
      <Card className="p-4 sm:p-5" as="section">
        ...
        <div className="grid grid-cols-3 gap-3">
          <Stat value="1h 06m" label="Debt · 14 nights" tone="warn" />
          <Stat value="7h 07m" label="Your nightly need" />
          <Stat value="+42m" label="Week trend" tone="warn" />
        </div>
      ...
      {/* Trend chart */}
      <Card className="p-4 sm:p-5">
        <SectionHeader title="Readiness — 30 days" />
      ...
      <p className="px-1 text-[11px] text-faint">
        ...
        <Link href="/health" className="underline-offset-2 hover:underline">Health</Link>. VO₂ max is {METRICS.vo2max.displayValue}.
      </p>
    </Page>
  );
}
...
function Stat({ value, label, tone }: { value: string; label: string; tone?: "warn" }) {
  return (
    <div className="rounded-lg border border-border bg-surface-2 p-3">
      <div className={`tnum text-lg font-semibold ${tone === "warn" ? "text-warn" : "text-text"}`}>{value}</div>
